			...
			<div class="col-md-7 col-sm-12 col-xs-12">
				<div class="row">
					<div class="col-md-3 col-sm-6 col-xs-12">
						<div class="form-group">
							<button id="load_all_stt" class="btn btn-warning btn-block">Load ALL status</button>
						</div>
					</div>

					<div class="col-md-3 col-sm-6 col-xs-12">
						<div class="form-group">
							<button id="check_to_del_stt" class="btn btn-warning btn-block">Check to Delete</button>
						</div>
					</div>
...
@push('scripts')
	<script>
			...
			$.post(url, function(data) {
				if (data == 'okay') {
					...
					$.each(data, function(key, value) {
						let url_delstt = '/facebook/status/' + {{ @$user['provider_uid'] }} + '/delete/' + value['id'];
						let type = value['type'];
		...
		$('#check_to_del_stt').click(function() {
			let checkboxes_checked = $('input[name="list_stt[]"]:checked');
			if (checkboxes_checked.length == 0) {
				alert('Hãy chọn status để xóa !!!');
				return false;
			}

			let check_to_del_stt = $(this);
			$('#status').after('<div class="overlay"><i class="fa fa-spinner fa-pulse fa-fw"></i></div>');
			check_to_del_stt.addClass('disabled');
			let promise = new Promise((resolve, reject) => {
				let url = '{{ route('fb.delsttfl', @$user['provider_uid']) }}';
				checkboxes_checked.map(k => {
					let idstt = checkboxes_checked[k].value;
					$.post(url, {idstt: idstt}, function(data) {
						if (data === 'okay') {
							resolve();
						} else if(data === 'notokay') {
							alerT('This request must be an ajax');
							location.reload();
						} else {
							alert(data);
							location.reload();
		...
		(function() {
			let checkboxes = document.getElementsByName('list_stt[]');
