<template>
   <v-list-tile class="sidebar-profile">
		...
		<v-list-tile-content class="ml-3">
			<v-list-tile-title><span>{{ $auth.user().username }}</span></v-list-tile-title>
		</v-list-tile-content>
		<v-menu 
			bottom
			offset-y
		...
		>
			...
			<div class="dropdown-content">
				<div class="dropdown-top white--text primary">
					...
					<span class="d-block fs-12 fw-normal">{{ $auth.user().email }}</span>
				</div>
				<v-list class="dropdown-list">
					...
					<template v-else>
						<v-list-tile @click="logoutUser($store, $auth)" :key="userLink.id">
							<i :class="userLink.icon"></i>
							<span>{{$t(userLink.title)}}</span>
						</v-list-tile>
					</template>
...
export default {
	...
	methods: {
		logoutUser(store, auth) {
			store.dispatch('logoutUser', auth).then(function() {
				store.dispatch('deleteFacebookAccount', auth)
			})
		},
		getMenuLink(path) {
